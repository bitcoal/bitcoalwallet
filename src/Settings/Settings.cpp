...
const char DEFAULT_WALLET_FILE_NAME[] = "bitcoalwallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("bitcoalwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("bitcoalwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("bitcoalwallet.desktop");
  ...
  res = autorunSettings.contains("BitCoalWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("BitCoalWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.bitcoal.bitcoal scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=BitCoal Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("BitCoalWallet", appPath);
    } else {
      autorunSettings.remove("BitCoalWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoal", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:bitcoal");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoal\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoal\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("bitcoalwallet.cfg"));
