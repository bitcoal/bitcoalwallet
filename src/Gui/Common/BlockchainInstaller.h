...
private:
  ...
  const QDir m_bitcoalDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfBitCoalBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _bitcoal) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo bitcoalBlockchainInfo() const;
