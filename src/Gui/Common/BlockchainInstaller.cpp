...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_bitcoalDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfBitCoalBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 bitcoalHeight;
  if (!checkIfBlockchainOutdated(currentHeight, bitcoalHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(bitcoalHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfBitCoalBlockchainExists() const {
  return m_bitcoalDir.exists() && m_bitcoalDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_bitcoalDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::bitcoalBlockchainInfo() const {
  return QFileInfo(m_bitcoalDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_bitcoalDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_bitcoalDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
